...
expSet <- list(
  ...
  GSE171681 = list(data = scale(t(exp_list$exp$GSE171681)))
)
checkSets(expSet)
```
...
```{r}
cox_results <- cox_coef(modules$multiMEs$GSE171680$data %>% t(),
                        GSE171682c,
                        time_var = "rfs_month",
                        event_var = "recur")
```
...
save(
  ...
  cox_results,
  cox_results_os,
  file = file.path(dir.results.wgcna,"WGCNA_GSE171682_results.rda")
)
```
...
```{r}
load(file.path(dir.results.wgcna,"WGCNA_GSE171682_results.rda"))
```
...
save(
  ...
  file = file.path(dir.results.wgcna, "WGCNA_key_modules_GSE171682_pairs.rda")
)
```

```{r}
load(file.path(dir.results.wgcna, "WGCNA_key_modules_GSE171682_pairs.rda"))
```

# Train organoid model
...
```{r}
# Get selected genes
selected_modules <- hub_gene_in_modules_drug[names(hub_gene_in_modules_drug) %in% c("lightyellow")]
  ...
  .fun = function(geo){
    ...
    results_df <- rbind(
      ...
      mutate(Model = c("ridge","ensemble","RF"),
             GEO = geo,
             gene_select_criteria = "Most variable GSE171680 WGCNA lightyellow",
             .before = 1)
